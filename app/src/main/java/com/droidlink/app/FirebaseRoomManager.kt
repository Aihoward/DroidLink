...
class FirebaseRoomManager {
                    ...
                    !committed || slot == null -> onError("Session is full (Host + Player 2 + Player 3 + Player 4)")
                        ...
                        disconnect.removeValue()
                            .addOnSuccessListener {
                                ...
                                val assignment = RemoteSlotAssignment(slot, joinerId)
                                updateClaimDisplayName(code, assignment, metadata.displayName)
    ...
    private fun updateClaimDisplayName(code: String, assignment: RemoteSlotAssignment, displayName: String) {
        ...
        claim.runTransaction(object : Transaction.Handler {
            override fun doTransaction(data: MutableData): Transaction.Result {
                if (data.child("joinerId").getValue(String::class.java) != assignment.joinerId) {
                    return Transaction.abort()
                }
                data.child("displayName").value = ProfilePolicy.normalizeDisplayName(
                    displayName,
                    "Player ${assignment.playerSlot}"
                )
                return Transaction.success(data)
            }
            ...
            override fun onComplete(error: DatabaseError?, committed: Boolean, snapshot: DataSnapshot?) {
                if (error != null) {
                    Log.w(TAG, "PROFILE_METADATA_SYNC_FAILED: slot=P${assignment.playerSlot}", error.toException())
                } else if (!committed) {
                    Log.d(TAG, "PROFILE_METADATA_SYNC_SKIPPED: slot=P${assignment.playerSlot} ownership changed")
                }
            }
        })
    }
