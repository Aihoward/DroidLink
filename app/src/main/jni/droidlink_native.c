...
JNIEXPORT jboolean JNICALL Java_com_droidlink_app_UinputVirtualGamepadBackend_nativeAxes(JNIEnv* e,jclass c,jlong h,jfloat lx,jfloat ly,jfloat rx,jfloat ry,jfloat lt,jfloat rt,jfloat dx,jfloat dy){int f=(int)h;int ok=emit(f,EV_ABS,ABS_X,sv(lx))&&emit(f,EV_ABS,ABS_Y,sv(ly))&&emit(f,EV_ABS,ABS_RX,sv(rx))&&emit(f,EV_ABS,ABS_RY,sv(ry))&&emit(f,EV_ABS,ABS_Z,tv(lt))&&emit(f,EV_ABS,ABS_RZ,tv(rt))&&emit(f,EV_ABS,ABS_HAT0X,(int)dx)&&emit(f,EV_ABS,ABS_HAT0Y,(int)dy)&&emit(f,EV_SYN,SYN_REPORT,0);return ok;}
